...
uv will not include `pip` in virtual environment by default. If you need it, use `--seed`:

```console
$ uv venv --seed
```

Note `--seed` may also include other seed packages in additon to `pip`. See the
